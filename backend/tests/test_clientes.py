...
# ---------------------------------------------------------------------------
# Sub-recursos ventas — GET /api/clientes/{id}/cotizaciones y /nota-ventas
# ---------------------------------------------------------------------------
...
def test_cotizaciones_cliente_filtra_y_ordena(client, admin_token, admin_user, db):
    from datetime import date
    c_a = client.post("/api/clientes/", json={"nombre": "Cliente Ventas A"}, headers={"Authorization": f"Bearer {admin_token}"}).json()
    c_b = client.post("/api/clientes/", json={"nombre": "Cliente Ventas B"}, headers={"Authorization": f"Bearer {admin_token}"}).json()

    c1 = _crear_cotizacion_db(db, c_a["id"], admin_user.id, numero=1,
                              fecha=date(2026, 1, 10), total="1000")
    c2 = _crear_cotizacion_db(db, c_a["id"], admin_user.id, numero=2,
                              fecha=date(2026, 2, 10), total="2000")
    _crear_cotizacion_db(db, c_b["id"], admin_user.id, numero=3, fecha=date(2026, 3, 10))

    r = client.get(f"/api/clientes/{c_a['id']}/cotizaciones", headers={"Authorization": f"Bearer {admin_token}"})
    assert r.status_code == 200
    data = r.json()
...
def test_cotizaciones_cliente_filtro_estado(client, admin_token, admin_user, db):
    c = client.post("/api/clientes/", json={"nombre": "Cliente Cot Estado"}, headers={"Authorization": f"Bearer {admin_token}"}).json()
    _crear_cotizacion_db(db, c["id"], admin_user.id, numero=1, estado="aceptada")
    _crear_cotizacion_db(db, c["id"], admin_user.id, numero=2, estado="rechazada")

    r = client.get(
        f"/api/clientes/{c['id']}/cotizaciones?estado=aceptada",
        headers={"Authorization": f"Bearer {admin_token}"},
    )
    assert r.status_code == 200
    data = r.json()
...
def test_nota_ventas_cliente_filtra_y_ordena(client, admin_token, db):
    from datetime import date
    c_a = client.post("/api/clientes/", json={"nombre": "Cliente NV A"}, headers={"Authorization": f"Bearer {admin_token}"}).json()
    c_b = client.post("/api/clientes/", json={"nombre": "Cliente NV B"}, headers={"Authorization": f"Bearer {admin_token}"}).json()

    n1 = _crear_nv_db(db, c_a["id"], numero=10, fecha=date(2026, 1, 5), total="500")
    n2 = _crear_nv_db(db, c_a["id"], numero=None, fecha=date(2026, 2, 5), total="700")
    _crear_nv_db(db, c_b["id"], numero=30)

    r = client.get(f"/api/clientes/{c_a['id']}/nota-ventas", headers={"Authorization": f"Bearer {admin_token}"})
    assert r.status_code == 200
    data = r.json()
...
def test_nota_ventas_cliente_filtro_estado(client, admin_token, db):
    c = client.post("/api/clientes/", json={"nombre": "Cliente NV Estado"}, headers={"Authorization": f"Bearer {admin_token}"}).json()
    _crear_nv_db(db, c["id"], numero=1, estado="facturada")
    _crear_nv_db(db, c["id"], numero=2, estado="pendiente")

    r = client.get(
        f"/api/clientes/{c['id']}/nota-ventas?estado=facturada",
        headers={"Authorization": f"Bearer {admin_token}"},
    )
    assert r.status_code == 200
    data = r.json()
...
def test_ventas_cliente_404_si_no_existe(client, admin_token):
    r = client.get("/api/clientes/99999/cotizaciones", headers={"Authorization": f"Bearer {admin_token}"})
    assert r.status_code == 404
...
def test_ventas_cliente_vendedor_403_si_no_asignado(client, admin_token, vendedor_token):
    c = client.post("/api/clientes/", json={"nombre": "Cliente Ajeno Ventas"}, headers={"Authorization": f"Bearer {admin_token}"}).json()
    r = client.get(f"/api/clientes/{c['id']}/cotizaciones", headers={"Authorization": f"Bearer {vendedor_token}"})
    assert r.status_code == 403
    r = client.get(f"/api/clientes/{c['id']}/nota-ventas", headers={"Authorization": f"Bearer {vendedor_token}"})
    assert r.status_code == 403
