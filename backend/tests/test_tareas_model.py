...
def test_tarea_creada_con_defaults(db, admin_user):
    t = Tarea(
...
def test_check_constraint_max_una_fk(db, admin_user, cliente_demo, empresa_demo):
    t = Tarea(
        titulo="Bad",
        due_date=date.today(),
        origen="manual",
...
def test_dedup_key_unique_parcial_solo_pendiente(db, admin_user):
    t1 = Tarea(
...
def test_regla_tarea_seed_placeholders(db):
    # Seed lives in Alembic migration (Task 2). Here we just verify the model is mappable.
