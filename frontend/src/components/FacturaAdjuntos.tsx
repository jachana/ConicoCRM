...
export default function FacturaAdjuntos({ facturaId, disabled = false }: { facturaId: number; disabled?: boolean }) {
  const qc = useQueryClient()
  const fileRef = useRef<HTMLInputElement>(null)
  const [confirmandoId, setConfirmandoId] = useState<number | null>(null)

  const { data: docs = [], isLoading } = useQuery<FacturaAdjunto[]>({
    queryKey: ['factura-adjuntos', facturaId],
    queryFn: () => api.get(`/api/facturas/${facturaId}/adjuntos/`).then(r => r.data),
  })
  ...
  const subir = useMutation({
    mutationFn: (file: File) => {
      ...
      fd.append('file', file)
      return api.post(`/api/facturas/${facturaId}/adjuntos/`, fd, {
  ...
  const eliminar = useMutation({
    mutationFn: (docId: number) => api.delete(`/api/facturas/${facturaId}/adjuntos/${docId}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['factura-adjuntos', facturaId] }),
  })

  function descargar(doc: FacturaAdjunto) {
    api.get(`/api/facturas/${facturaId}/adjuntos/${doc.id}/download`, { responseType: 'blob' })
      .then(r => {
