...
interface PreviewError { fila: number; sku: string | null; nombre: string | null; motivo: string }
interface PreviewFila { fila: number; sku: string; nombre: string; accion: 'crear' | 'actualizar' }
...
interface ImportDetalle { fila: number; sku: string | null; nombre: string | null; estado: Estado; motivo: string | null }
...
export function ProductosImportSection() {
  ...
  async function handlePreview() {
    ...
    try {
      ...
      const resp = await api.post('/api/productos/import/preview', fd, {
  ...
  async function handleImport() {
    ...
    try {
      const fd = new FormData()
      fd.append('archivo', file)
      const resp = await api.post<ImportResp>('/api/productos/import', fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      setResult(resp.data)
      setStep('done')
  ...
  async function handleDownloadTemplate() {
    try {
      const resp = await api.get('/api/productos/import/template', { responseType: 'blob' })
  ...
  async function handleDownloadReport() {
    if (!result) return
    try {
      const resp = await api.post('/api/productos/import/report', result, { responseType: 'blob' })
      const url = URL.createObjectURL(resp.data as Blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'reporte_import_productos.xlsx'
      a.click()
      URL.revokeObjectURL(url)
    } catch {
    ...
    <div className="space-y-4">
      <Card padded>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
          Descarga la plantilla, completa los productos y vuelve a subir el archivo. Columnas requeridas:{' '}
          <code className="font-mono bg-gray-100 dark:bg-gray-800 px-1 rounded">sku</code>,{' '}
          <code className="font-mono bg-gray-100 dark:bg-gray-800 px-1 rounded">nombre</code>. Opcionales:
          descripcion, precio_costo, precio_venta, cantidad_minima, cantidad_maxima, unidad, categoria. El match con
          productos existentes es por SKU normalizado.
        </p>
            ...
            <div className="space-y-1">
              ...
              {preview.errores.map((e, i) => (
                <div key={i} className="flex items-start gap-2 text-xs text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded px-2 py-1.5">
                  ...
                  <span>Fila {e.fila}{e.sku ? ` (${e.sku})` : ''}: {e.motivo}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {step === 'preview' && preview && preview.filas_validas > 0 && (
        <div className="flex justify-between items-center">
          ...
          <Button onClick={handleImport} disabled={busy}>
            {busy ? 'Importando...' : `Importar ${preview.filas_validas} producto${preview.filas_validas !== 1 ? 's' : ''}`}
          </Button>
        </div>
      )}
            ...
            {result.detalles.map((d, i) => {
              ...
              return (
                <div key={i} className={`flex items-start gap-2 text-xs rounded px-2 py-1.5 ${s.cls}`}>
                  ...
                  {d.sku && <span className="font-mono shrink-0">{d.sku}</span>}
                  {d.nombre && <span className="truncate text-gray-600 dark:text-gray-400">{d.nombre}</span>}
