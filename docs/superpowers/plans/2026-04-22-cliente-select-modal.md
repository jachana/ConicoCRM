# Cliente Select Modal — Cotización Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** When selecting an empresa in a cotización (without a cliente already set), auto-open a modal that lists existing clientes for that empresa and allows creating a new one.

**Architecture:** New `ClienteSelectModal.tsx` component handles two views (list/create) with no new backend endpoints. `CotizacionDetalle.tsx` gains a `useEffect` that opens the modal when `empresaId` changes while `clienteId` is empty, plus a `UserPlus` re-open button.

**Tech Stack:** React 18 + TypeScript + Tailwind CSS + TanStack Query + Lucide icons + Sonner toasts
...
| File | Action |
|------|--------|
| `frontend/src/components/ClienteSelectModal.tsx` | CREATE — new modal component |
| `frontend/src/pages/CotizacionDetalle.tsx` | MODIFY — state, useEffect, button, handler, render |

---

### Task 1: Create `ClienteSelectModal.tsx`

**Files:**
- Create: `frontend/src/components/ClienteSelectModal.tsx`

This modal has two internal views: `'list'` (shows clientes for the empresa) and `'create'` (full cliente form with empresa locked).

- [ ] **Step 1: Create the file with list view**
...
```bash
git add frontend/src/components/ClienteSelectModal.tsx
git commit -m "feat: add ClienteSelectModal component (list + create views)"
```

---

### Task 2: Integrate modal into CotizacionDetalle

**Files:**
- Modify: `frontend/src/pages/CotizacionDetalle.tsx`

Four changes: (a) add import, (b) add `clienteModalOpen` state, (c) add `useEffect` trigger, (d) add `handleClienteSelect`, (e) add `UserPlus` button next to empresa selector, (f) render `<ClienteSelectModal>`.

- [ ] **Step 1: Add import for ClienteSelectModal and UserPlus icon**

In `CotizacionDetalle.tsx` line 6, `UserPlus` needs to be added to the Lucide import and `ClienteSelectModal` imported.
...
After line 11 (after the `UnsavedChangesModal` import), add:
```tsx
import ClienteSelectModal from '../components/ClienteSelectModal'
```

- [ ] **Step 2: Add clienteModalOpen state**

After line 143 (`const [unsavedModal, setUnsavedModal] = useState(false)`), add:
```tsx
const [clienteModalOpen, setClienteModalOpen] = useState(false)
```

- [ ] **Step 3: Add useEffect to auto-open modal when empresa selected without cliente**

After the existing useEffects (around line 245, after the block that loads cotización data), add:
```tsx
useEffect(() => {
...
- [ ] **Step 4: Add handleClienteSelect function**

After `handleEmpresaChange` (around line 307), add:
```tsx
...
- [ ] **Step 5: Add UserPlus button next to empresa selector**

Current empresa `<div>` block (lines 829-840):
```tsx
              <div>
...
- [ ] **Step 6: Render ClienteSelectModal**

Near the bottom of the JSX return, after the existing `<CreditWarningModal ... />` usage (around line 1144), add:
```tsx
      <ClienteSelectModal
...
git commit -m "feat: integrate ClienteSelectModal into CotizacionDetalle (auto-open + UserPlus re-open)"
```
